...
public:
    ...
    static void Datalog(std::ofstream& csv_writer_, std::_Put_time<char> datetime, std::chrono::duration<double> time_elapsed, const std::vector<Eigen::VectorXd> &data_vectors)
    {
        if (!csv_writer_.is_open()) return;

        // Start writing the timestamp and elapsed time
        csv_writer_ << datetime << "," << time_elapsed.count(); // datetime, time [s]
    ...
    static Eigen::VectorXd MAvg(const Eigen::VectorXd &vec, std::deque<Eigen::VectorXd> &buffer, std::size_t window_size) {
        ...
        if (buffer.size() == window_size) {
            Eigen::VectorXd vec_mavg = Eigen::VectorXd::Zero(vec.size());
            for (const auto& v : buffer) {
                vec_mavg += v;
            }
            vec_mavg /= window_size;
            return vec_mavg;
        } 
        // If buffer is not full, return the current vector
        else {
            return vec;
        }
    }
